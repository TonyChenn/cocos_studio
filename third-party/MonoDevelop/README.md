...
这不是上层应用自有实现，也不是直接使用 MonoDevelop 官方源码替换定制版本。
...
- 项目文件名使用 `.Restored.csproj`，程序集名称仍为 `MonoDevelop.Debugger` / `MonoDevelop.SourceEditor2` / `CocoStudio.WindowsPlatform`。
  文件名刻意不同，避免根构建按项目文件名扫描时，自动停止复制回退构建所需的旧 DLL。
- 按用户要求，`UseRestoredMonoDevelop` 现默认启用，普通 Debug/Release 构建使用这三份恢复源码及新版 Mono.Debugging。
  `build/RestoredMonoDevelop.Test.props` 仍可用于隔离测试，将输出限定为
  `bin/RestoredMonoDevelopTest/`，中间文件限定为 `obj/RestoredMonoDevelopTest/`。
- 主程序通过条件 ProjectReference 引用 SourceEditor2，后者引用 Debugger；`.sln` 和 `.slnx` 都沿此依赖构建，
  无需在方案中重复添加恢复项目。
...
- 源码模式下所有项目使用同一个 NuGet `Mono.Debugging 1.0.20170212.42`；NRefactory、Mono.Addins、Xwt、Gtk# 等
  继续采用现有 NuGet 版本。Core、Ide、Mono.TextEditor、DesignerSupport 暂时引用仓库原 DLL。
...
- 根 ReferencePath 的旧 DLL 搜索顺序早于 HintPath，因此源码构建目标明确把 Mono.Debugging 包目录放在前面。
  同时排除旧四库的统一复制及旧 Debugger/SourceEditor2/WindowsPlatform 的间接复制，最后核对实际输出哈希。
- 普通构建使用 `bin/Debug` 或 `bin/Release`；启用隔离测试模式时，构建前和清理前仍验证测试输出路径。
- `dlls` 中四份原库继续保留。需要回退时，对主方案传 `/p:UseRestoredMonoDevelop=false` 并还原包、完整重建，
...
当前验证：两种方案均为 0 错误、163 警告；输出的三份恢复库与各自编译产物哈希一致，
Mono.Debugging 与指定 NuGet 包一致。351 处成员引用、85 项资源、定制接口、断点样本双向互读、
...
`-Mode old` 只切回旧 Mono.Debugging/Debugger/SourceEditor2，WindowsPlatform 仍使用修复后的同一候选，
