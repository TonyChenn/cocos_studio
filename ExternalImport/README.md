...
全部新增代码位于 `ExternalImport/`，使用独立解决方案 `CocosStudio.ExternalImport.sln`。原 `CocosStudio.sln`、`Program.cs`、`CocoStudio.Core` 和现有模块源码均不需要修改。
...
```text
ExternalImport\CocosStudio.ExternalImport.sln
```

默认产物目录：

```text
ExternalImport\artifacts\Debug\Client\CocosStudio.Import.exe
ExternalImport\artifacts\Debug\StudioPlugin\CocosStudio.ExternalImport.StudioPlugin.dll
```

构建完成后安装 Debug 插件：

```powershell
.\ExternalImport\Install-Plugin.ps1 -Configuration Debug
```
